...
{
    /// <summary>
    /// A fast Producer Consumer implementation for 16bit
    /// images using a fixed ring-buffer
    /// </summary>
    public unsafe class PrCoImageRingBuffer16 : IDisposable
    {
        #region Fields
        ...
        public PrCoImageRingBuffer16(int imageWidth, int imageHeight, int nImages)
        {
            if (nImages < 1)
            ...
            _imageStride = (int)(4 * Math.Ceiling(_imageWidth * 2 / 4.0));
            //Allocate sub-buffers
            for (int i = 0; i < _nImages; i++)
            {
                _buffers[i] = (ushort*)Marshal.AllocHGlobal(_imageStride * _imageHeight).ToPointer();//note that stride is number of bytes, not pixels!
